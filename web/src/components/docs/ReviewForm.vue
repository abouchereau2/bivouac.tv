...
<template>
  <div v-if="authStore.isAuthenticated" class="bg-white dark:bg-slate-800 rounded-lg p-6 border border-slate-200 dark:border-slate-700">
    <h3 class="text-lg font-semibold text-slate-900 dark:text-white mb-4">
      {{ isEditing ? t('review.editTitle') : t('review.createTitle') }}
    </h3>
    ...
    <form @submit.prevent="handleSubmit">
      ...
      >
        {{ isSubmitting ? t('review.saving') : (isEditing ? t('review.updateButton') : t('review.submitButton')) }}
      </button>
    </form>
  </div>
