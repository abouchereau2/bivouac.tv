...
<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex items-center gap-3 mb-8">
      ...
      <h1 class="text-3xl font-bold text-slate-900 dark:text-white">{{ t('watchlist.title') }}</h1>
    </div>
    ...
    <div v-else-if="watchlist.length === 0" class="text-center py-12">
      ...
      <h2 class="text-xl font-semibold text-slate-900 dark:text-white mb-2">{{ t('watchlist.empty') }}</h2>
      <p class="text-slate-500 dark:text-slate-400 mb-4">{{ t('watchlist.emptyDescription') }}</p>
      <RouterLink :to="localePath('/browse')" class="text-blue-600 hover:text-blue-700 font-medium">
        {{ t('watchlist.browseButton') }}
      </RouterLink>
    </div>
